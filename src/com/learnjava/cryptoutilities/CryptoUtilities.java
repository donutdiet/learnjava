...
public final class CryptoUtilities {
    ...
    /**
     * stores the number of witnesses used to test for compositeness.
     */
    private static final int NUM_WITNESS_CANDIDATES = 50;
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // check whether the last digit has a remainder when dividing by 2
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        /*
         * n^p mod m except you mod at every step of the recursion
         */
        ...
        // copy of p that will be modified
        ...
        if (pTemp.compareTo(one) == 0) {
            // num^1 always = num
            pTemp.add(zero);
        } else {
            // check if p is even
            if (isEven(pTemp)) {
                // divide p by 2 for faster power algorithm
                pTemp.divide(two);
                powerMod(n, pTemp, m);
                NaturalNumber nCopy = new NaturalNumber2(n);
                // results in n^(p/2)*n^(p/2)
                n.multiply(nCopy);
                n.transferFrom(n.divide(m));
            } else {
                // decrement p so it becomes even
                pTemp.decrement();
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean witnessToCompositeness = false;
        ...
        if (wCopy1.compareTo(one) == 0) {
            // if it does then w is a witness to n being composite (not prime)
            witnessToCompositeness = true;
        }
        ...
        if (wCopy2.compareTo(one) != 0) {
            // if it doesn't then w is a witness to n being composite (not prime)
            witnessToCompositeness = true;
        }

        return witnessToCompositeness;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // create copy of n
        NaturalNumber nCopy = new NaturalNumber2(n);
        // subtract 2 so that w < n-1
        nCopy.subtract(two);

        // generate 50 witness candidates to test
        for (int i = 0; i < NUM_WITNESS_CANDIDATES; i++) {
            NaturalNumber random = randomNumber(nCopy);
            // necessary to meet the w > 1 requirement
            if (random.compareTo(zero) == 0 || random.compareTo(one) == 0) {
                random = new NaturalNumber2(2);
            }
            // check if the randomly generated NN is a witness to n being composite
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // always run the if and else blocks before checking while condition
        do {
            if (isEven(n)) {
                // increment even numbers by 1 instead of 2
                n.increment();
            } else {
                // even numbers are not prime
                n.add(two);
            }
            // keep looping if n is not prime
        } while (!isPrime2(n));
    }
